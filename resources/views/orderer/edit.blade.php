                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ isset($page) ? route('orderer.update', [$orderer->id, 'page' => $page]) : route('orderer.update', $orderer->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <x-input class="block mt-1" type="text" name="name" :value="old('name', $orderer->name)" autofocus />
                        </div>
                        <div class="mt-4">
                            <x-label for="sort_order" :value="__('並び順')" />
                            <x-input class="block mt-1" type="text" name="sort_order" :value="old('sort_order', $orderer->sort_order)" />
                        </div>
                        <div class="mt-4">
                            <x-label for="display" :value="__('表示 / 非表示')" />
                            <input type="radio" name="display" value="1"{{ old('display', $orderer->display) == '1' ? ' checked' : '' }}>{{ __('表示') }}
                            <input type="radio" name="display" value="0"{{ old('display', $orderer->display) == '0' ? ' checked' : '' }}>{{ __('非表示') }}
                        </div>
                        <div class="mt-4">
                            <a href="{{ isset($page) ? route('orderer.index', ['page' => $page]) : route('orderer.index') }}" class="text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-gray-600 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-800">{{ __('戻る') }}</a>
