                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('project.update', $projectInfo['project']->id)}}" method="POST">
                        ...
                        <div>
                            ...
                            <x-input class="block mt-1 w-full" type="text" name="name" :value="old('name', $projectInfo['project']->name)" autofocus />
                        </div>
                        <div class="mt-4">
                            ...
                            <select class="block mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="crowd_sourcing_id">
                                <option value=""{{ old('crowd_sourcing_id', $projectInfo['project']->crowd_sourcing_id) == '' ? ' selected' : '' }}></option>
                            @foreach($projectInfo['crowd_sourcings'] as $crowd_sourcing)
                                <option value="{{ $crowd_sourcing->id }}"{{ old('crowd_sourcing_id', $projectInfo['project']->crowd_sourcing_id) == $crowd_sourcing->id ? ' selected' : '' }}>{{ $crowd_sourcing->name }}</option>
                            @endforeach
                            </select>
                        </div>
                        <div class="mt-4">
                            ...
                            <select class="block mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="orderer_id">
                                <option value=""{{ old('orderer_id', $projectInfo['project']->orderer_id) == '' ? ' selected' : '' }}></option>
                            @foreach($projectInfo['orderers'] as $orderer)
                                <option value="{{ $orderer->id }}"{{ old('orderer_id', $projectInfo['project']->orderer_id) == $orderer->id ? ' selected' : '' }}>{{ $orderer->name }}</option>
                            @endforeach
                            </select>
                        </div>
                        <div class="mt-4">
                            ...
                            <select class="block mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="progress_id">
                                <option value=""{{ old('progress_id', $projectInfo['current_progress_id']) == '' ? ' selected' : '' }}></option>
                            @foreach($projectInfo['progresses'] as $progress)
                                <option value="{{ $progress->id }}"{{ old('progress_id', $projectInfo['current_progress_id']) == $progress->id ? ' selected' : '' }}>{{ $progress->name }}</option>
                            @endforeach
                            </select>
                        </div>
                        <div class="mt-4">
                            <x-label for="publication_on" :value="__('掲載日')" />
                            <input class="block mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="date" name="publication_on" value="{{ old('publication_on', $projectInfo['project']->publication_on) }}" />
                        </div>
                        <div class="mt-4">
                            <x-label for="application_deadline_on" :value="__('応募期限')" />
                            <input class="block mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="date" name="application_deadline_on" value="{{ old('application_deadline_on', $projectInfo['project']->application_deadline_on) }}" />
                        </div>
                        <div class="mt-4">
                            <x-label for="contract_amount_excluding_tax" :value="__('契約金額(税抜)')" />
                            <x-input class="block mt-1" type="text" name="contract_amount_excluding_tax" :value="old('contract_amount_excluding_tax', $projectInfo['project']->contract_amount_excluding_tax)" />
                        </div>
                        <div class="mt-4">
                            <x-label for="display" :value="__('表示 / 非表示')" />
                            <input type="radio" name="display" value="1"{{ old('display', $projectInfo['project']->display) == '1' ? ' checked' : '' }}>{{ __('表示') }}
                            <input type="radio" name="display" value="0"{{ old('display', $projectInfo['project']->display) == '0' ? ' checked' : '' }}>{{ __('非表示') }}
                        </div>
                        <div class="mt-4">
